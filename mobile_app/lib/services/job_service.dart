...
class JobService {
  ...
  Future<List<dynamic>> getJobRecommendations() async {
    _clearErrorMessage();
    try {
      ...
      final response = await http.get(
        Uri.parse('$baseUrl/api/jobs/recommendations'),
        headers: {
      ...
      if (errorMessage.contains('TimeoutException')) {
        ...
        _setErrorMessage('获取职位推荐失败: $errorMessage');
      }
      return [];
    }
  }
  ...
  Future<bool> applyForJob(int jobId) async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('$baseUrl/api/jobs/apply'),
        headers: {
      ...
      if (errorMessage.contains('TimeoutException')) {
        ...
        _setErrorMessage('申请职位失败: $errorMessage');
      }
      return false;
    }
  }
  ...
  Future<List<dynamic>> searchJobs(String keyword) async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('$baseUrl/api/jobs/search?keyword=$encodedKeyword'),
        headers: {
          'Authorization': 'Bearer $token',
        },
      ).timeout(
      ...
      if (errorMessage.contains('TimeoutException')) {
        ...
        _setErrorMessage('搜索职位失败: $errorMessage');
      }
      return [];
    }
  }
